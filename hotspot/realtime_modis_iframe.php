...
// 			  });
    ...
    <div class="container">
							...
							<div class="panel panel-default">
								...
								<div id="collapseOne" class="panel-collapse collapse in">
									<div class="panel-body">
										<form class="form-horizontal" id="realtime_modis_iframe" name="realtime_modis_iframe" method="POST" action="">
		...
		<div class="row">
			<div class="col-md-12">
				<h3 class="text-center">กราฟแสดงจุดความร้อนประจำวัน</h3>
				<h3 class="text-center">บริเวณพื้นที่ในจังหวัด<?php echo $province; ?></h3>
				<div id="ch_hotspot" style="min-width: 310px; height: 400px; margin: 0 auto"></div>
				<p class="text-center">
					<button class="btn btn-success" id="export1"><i class="glyphicon glyphicon-download-alt"></i> Export CSV</button>	
				</p>
				<div class="clearfix"></div>
			</div>
		</div>
		...
		<div class="row">
			<div class="col-md-12">
				<h3 class="text-center">กราฟแสดงจุดความร้อนแยกตามอำเภอ</h3>
				<h3 class="text-center">ตั้งแต่วันที่ <?=ConvertRToThaiDate(date('2017-01-01'),date('Y-m-d'))?></h3>
				<h3 class="text-center">บริเวณพื้นที่ในจังหวัด<?php echo $province; ?></h3>
				<div id="ch_hotspot2" style="min-width: 310px; height: 400px; margin: 0 auto"></div>
				<p class="text-center">
					<button class="btn btn-success" id="export2"><i class="glyphicon glyphicon-download-alt"></i> Export CSV</button>	
				</p>
				<div class="clearfix"></div>
			</div>
		</div>
	...
	function toggleChevron(e) {
		...
		$('#province').on('change',function(){
			if( this.value ){
                $.get( "getAmphur.php?p_name="+this.value, function( data ) {
					if(data){
						$("#amphur").html(data);
					}
				});
	...
	<script>
	var url = 'hot_spot.php?filename=<?=$province?>&callback=?';
	var chart1='';
	$.getJSON(url, function (data) {
		// create the chart
		chart1 = new Highcharts.stockChart('ch_hotspot', {
			chart: {
				alignTicks: false
			},
			rangeSelector : {
	...
	var chart2 = Highcharts.chart('ch_hotspot2', {
		chart: {
			type: 'column'
		},
		title: {
		...
		xAxis: {
			categories: [
			<?php 
				$sql="SELECT amphur.AMPHUR_ID,amphur.AMPHUR_NAME FROM `province` left join amphur on province.PROVINCE_ID = amphur.PROVINCE_ID where province.PROVINCE_NAME LIKE '%".$province."%'  order by amphur.AMPHUR_ID ASC";
				$result = mysql_query($sql, $conn);
				$i=0;
				while ($row = mysql_fetch_array($result)) {
					$i++;
		...
		tooltip: {
			...
			pointFormat: '<tr><td style="color:{series.color};padding:0">{series.name}: </td>' +
				'<td style="padding:0"><b>{point.y} spot</b></td></tr>',
			footerFormat: '</table>',
			shared: true,
			useHTML: true
		},
		...
		series: [{
			...
			<?php 
				...
				$i=0;
				while ($row = mysql_fetch_array($result)) {
					$sql2="SELECT count(modis.viirs_id) as hotspot 
					FROM modis 
					...
					AND modis_meta.raw_json LIKE '%".$province."%' 
					AND modis_meta.raw_json LIKE '%".trim($row[1])."%' ";
